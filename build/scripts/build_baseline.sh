#!/bin/bash -eu
# Build + Run + Coverage for baseline (libFuzzer)
...
if [[ ! -x "$OUT_BIN" || "${FORCE_BUILD:-0}" = "1" ]]; then
  echo "Building libFuzzer-style baseline -> $OUT_BIN"

  echo "Compiling sqlite3.c..."
  $CC $COV_FLAGS $COMMON_DEFS -I"$DEPS_DIR" -c "$DEPS_DIR/sqlite3.c" -o "$OBJ_DIR/sqlite3.o"

  echo "Compiling ossfuzz.c..."
  $CC $COV_FLAGS $COMMON_DEFS -I"$DEPS_DIR" -c "$FUZZER_DIR/ossfuzz.c" -o "$OBJ_DIR/ossfuzz.o"

  echo "Linking $OUT_BIN with libFuzzer..."
  $CXX $COV_FLAGS $FUZZ_FLAGS "$OBJ_DIR/sqlite3.o" "$OBJ_DIR/ossfuzz.o" $LDLIBS -o "$OUT_BIN"
  chmod +x "$OUT_BIN"
else
...
if [ -f "$SQLITE_SRC" ]; then
  ...
  IGNORE='(test_main|fuzzers/baseline/fuzz\.c|fuzzers/.*/test_main|fuzzers/.*/ossfuzz|test_main)'
